...
> MOZA is not a static application — it is a system that will evolve itself.
> This section defines HOW that evolution is governed to remain safe, auditable, and aligned with human intent.

### How Changes Are Proposed

1. **Proposal Format** — Every change MUST be documented as a structured proposal (see `Evolution Backlog` in `PROJECT_STATE.md §8.3`)
2. **Proposal Sources** — Human, Agent self-analysis, Research discovery, Open-source analysis, Benchmark gap identification
3. **Proposal Lifecycle**: `Draft → Review → Accepted/Rejected → Implemented → Certified → Merged`
4. **Proposal Registry** — All proposals tracked in GitHub Issues with label `evolution-proposal`

### How Capabilities Are Promoted

The promotion pipeline follows the **Three-Tier Deployment Pipeline**:
...
| Tier | Gate | Who Decides |
|------|------|-------------|
| **Experimental** | None | Agent autonomy |
| **Candidate** | 100% Canonical Benchmarks + all tests pass | Automated |
| **Stable** | Human manager approval (`manager-approve`) | Human only |

### How Deprecated Features Are Handled

1. **Deprecation Notice** — Marked in source with `@deprecated` and a migration path documented
2. **Grace Period** — Feature remains available for 2 full evolution cycles
3. **Telemetry Warning** — Active users of deprecated features are warned via telemetry
4. **Automatic Migration** — Where possible, MOZA auto-migrates usage to the replacement
5. **Removal** — Only after grace period AND zero active dependents

### How Backward Compatibility Is Guaranteed

1. **Regression Freeze** — All certified Canonical Benchmarks are frozen forever (see `REGRESSION_FREEZE.md`)
2. **Contract Testing** — Every capability has a Capability Contract that must never break
3. **API Versioning** — Public APIs are versioned (`/v1/`, `/v2/`); internal APIs follow semver
4. **Adapter Pattern** — Breaking changes are introduced behind adapters; old interfaces are wrapped, not replaced
5. **Migration Tools** — Automated migration scripts for any breaking change

### Who Approves Architectural Changes

| Change Type | Approver | Criteria |
|-------------|----------|----------|
| Bug fix (no API change) | Agent (auto) | Tests pass |
| New plugin/capability | Agent + Human review | Capability Contract + Benchmark |
| Architecture change | Human only | ADR document + Impact Analysis |
| Self-modification to governance | Human only | Supermajority review |
| Deletion/rewrite of certified capability | **BLOCKED** | No one — certified capabilities are frozen |

**Escalation Path:**
1. Agent proposes → Automated checks
2. If risk L0-L2 → Auto-approve with log
3. If risk L3-L4 → Human approval required
4. If architecture change → ADR + Human approval
5. If governance change → Human only, with full rationale

### Why Evolution Governance Matters Now

When MOZA itself begins proposing modifications to its own codebase, these governance rules become the **constitution** that prevents:
- Unregulated self-modification
- Silent regression of certified capabilities
- Architectural drift without oversight
- Removal of features users depend on

This section must be **locked** — amendments require explicit human consensus.
