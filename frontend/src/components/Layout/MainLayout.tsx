...
import { connectWebSocket, disconnectWebSocket, onBackendStatus } from "@/lib/api";
...
export default function MainLayout({ leftPanel, rightPanel, inputArea, agentStatus }: Props) {
  ...
  useEffect(() => {
    connectWebSocket();
    const unsubscribe = onBackendStatus((status) => {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100">
      ...
      <aside className="flex w-[250px] shrink-0 flex-col border-r border-slate-800/60 bg-slate-900/40">
        <div className="p-4 flex flex-col gap-4 border-b border-slate-800/40">
          {/* Logo Section: Large, Full Width, Centered */}
          <div className="flex flex-col items-center gap-2 w-full">
            <img src="/logo.png" alt="MOZA" className="w-full max-w-[200px] h-auto object-contain" />
            <span className="font-bold text-lg text-slate-300">MOZA OS</span>
          </div>
          {/* New Session Button: Full Width, matching logo width */}
          <button
        ...
        <div className="flex-1 overflow-y-auto px-3">
          <div className="mb-2 px-1 text-[10px] font-medium uppercase tracking-widest text-slate-400">
            Recent Sessions
          </div>
          <div className="flex flex-col gap-1">
            {RECENT_SESSIONS.map((s) => (
              <button
                key={s.id}
                className="flex items-center justify-between rounded-lg px-3 py-2 text-left text-sm text-slate-300 transition-colors hover:bg-slate-800/60 hover:text-white"
              >
                <span className="truncate">{s.label}</span>
                <span className="shrink-0 text-[10px] text-slate-500">{s.ts}</span>
              </button>
            ))}
          </div>
        </div>
